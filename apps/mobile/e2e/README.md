...
- **`offline_loop.py`** (15체크) — 오프라인 읽기 캐시(D21): 가입→홈(캐시 워밍) → persist 캐시 키가 userId 스코프·내용에 `skill-tree` 포함·`league` 제외 → `set_offline(True)` 배너 표시 → SPA 탭 이동으로 캐시 열람(빈 화면/무한 로딩 아님) → 학습 진입 차단(`offline-blocked`) → `set_offline(False)` 배너 사라짐 → A 로그아웃 시 캐시 제거→B 가입 시 다른 캐시 키.
