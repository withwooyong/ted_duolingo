...
- **`learning_loop.py`** (62체크) — 가입 → 온보딩 → 레슨(5종 전부 정답) → 완료(XP·스트릭) → 홈 반영 → 오답 하트 감소 → 리그·프로필·설정 → 언어 추가 페이월 → mock 구독 → 언어 전환(ko↔ja).
- **`review_loop.py`** (9체크) — SM-2 복습: 레슨 완료 → `user_review_state.due_at` 백데이트(psql) → 홈 복습 배너 → `/review` 세션(due 6문제 정답) → 완료(+5 XP) → 홈 반영(총 XP 20·일일 목표 15 유지·배너 사라짐). 복습 due는 첫 정답이 +1일 뒤라 백데이트로 강제한다.
