...
pnpm build:web        # 웹 정적 export (= expo export -p web → apps/mobile/dist/). PWA 셸 캐시 검증·배포용
...
# policies/ 아래 SQL은 번호 순서대로 전부 적용 (0002: 리그·배지, 0003: content_drafts API 차단, 0004: 복습 상태 본인 행, 0005: 복습 세션 멱등 가드 본인 행)
```
...
- **dev web(Expo web)은 서비스워커가 없어 오프라인 full reload(번들 재요청)가 불가** — Metro가 번들을 메모리에서 동적 제공하기 때문. 그래서 `offline_loop.py`는 reload 대신 SPA 내 탭 이동으로 캐시 열람을 검증하고, persist 기록은 localStorage 캐시 내용(`skill-tree` 포함)으로 확인한다. 실제 오프라인 reload 복원은 네이티브(임베드 번들) 또는 **PWA(`pnpm build:web`으로 정적 export + `public/sw.js` 셸 캐시, D23)**에서만 — `pwa_offline_reload.py`가 `dist/` 서빙으로 이 경로를 검증한다. SW(`components/sw-register.tsx`)는 **production export에서만 등록**(`process.env.NODE_ENV==='production'` 가드) — dev는 캐시할 해시 번들이 없어 등록하면 오작동
- **`public/`는 export 시 `dist/` 루트로 복사**(dev에서도 루트 서빙) — `public/sw.js`→`/sw.js`, `public/manifest.json`→`/manifest.json`, `public/icon-*.png`. **web `output:"single"`은 `+html.tsx`를 무시**하므로 HTML 셸 커스터마이즈(manifest 링크·meta)는 `public/index.html`로 한다(Expo가 CSS/JS 해시 링크를 `</head>`/`</body>`에 자동 주입)
- **onlineManager 기본값은 `online:true`이고 이벤트로만 갱신** — PWA가 오프라인 상태로 full reload되면 `offline` 이벤트가 안 와서 앱이 온라인으로 착각한다(배너 미표시·쿼리 일시정지 안 됨). `lib/online-status.ts`의 `initOnlineManager`가 웹에서 초기 상태를 `navigator.onLine`으로 한 번 동기화해 해결
...
- **오프라인 복습 큐(D24, D22의 복습 확장)**: 복습까지 오프라인화 — 레슨 D22와 대칭이되 두 가지가 다르다. ① **시점 동결 스냅샷**: live `review-session`(gcTime:0·persist 제외, 온라인 정확성 유지)과 **분리된** 영속 쿼리 `review-snapshot`(`hooks/use-review.ts`의 `useReviewSnapshot`/`reviewSnapshotKey`)을 둬 온라인일 때 due 세션을 동결한다. 홈이 "이어하기" 레슨처럼 prefetch하고, 오프라인 due 카운트·진입 게이팅은 스냅샷 길이 기준. **스냅샷은 `staleTime:0`** — 온라인이면 진입/복귀마다 최신 due로 재동결하고 동결은 오프라인(paused)일 때 자연히 일어난다(이걸 안 하면 persist된 stale-empty 캐시가 굳어 오프라인 배너가 안 뜬다 — 실제로 밟은 함정). ② **쓰기**: 복습 입력(pairId·history·정오답)을 `lib/sync-queue.ts`의 `reviews` 배열(enqueueReview)에 적재, `SyncProcessor`가 레슨 드레인 뒤 `lib/review-writes.ts`의 `completeReviewWrite`(온라인 훅 `useCompleteReview`도 호출하는 단일 소스)를 fresh profile에 대고 재실행. ③ **멱등성**: 복습은 레슨의 `user_progress` 같은 가드가 없어(SM-2·XP가 read-modify-write라 재실행 시 이중 적용) **새 테이블 `user_review_session`**(클라이언트 생성 id, RLS 0005)을 가드로 신설 — 같은 sessionId면 통째 skip. ④ 오프라인 완료는 홈 **낙관 갱신**(총 XP만 — 복습은 일일/주간 제외, `review-snapshot`을 빈 세션으로 소진→배너 숨김, "동기화 대기" pill은 레슨+복습 합산) + 완료 화면 `review-pending-note`, 복귀 시 invalidate(`review-snapshot` 포함) 보정. **복습 XP 계산은 `@ted/shared` `reviewXp` 순수함수**(낙관·서버 동일 소스). 한계는 D22와 동일(부분 실패 시 후속 쓰기 손실 가능하나 멱등 가드로 이중 적용 없음)
- **PWA 앱 셸 캐시(D23, 오프라인 full reload 복원)**: D21(데이터)·D22(쓰기)에 더해, 오프라인에서 **page reload 시 앱이 부팅**되게 하는 마지막 조각. 정적 export(`pnpm build:web` → `dist/`) + 서비스워커(`public/sw.js`)가 **앱 셸(index.html + 해시 JS/CSS 번들)만** 캐시한다. 역할 분리 유지 — SW는 **교차 오리진(Supabase)·비-GET을 통과**시켜 데이터는 persist(D21)·쓰기는 sync-queue(D22)가 담당. SW 전략: 네비게이션 network-first(오프라인이면 캐시된 셸 `/` fallback) / `/_expo/static/`·`/assets/` 해시 에셋 cache-first(불변) / 그 외 same-origin GET network-first. 셸 캐시 버전 `SHELL_CACHE`는 `query-client.ts`의 `CACHE_BUSTER`(RQ 데이터 shape)와 **독립**. 등록은 production export만(`components/sw-register.tsx`). dev web은 여전히 오프라인 reload 불가(SW 없음). 한계: 단일 번들이라 미방문 라우트 청크 이슈는 없지만, persist가 플러시되기 전(쓰로틀 1s) 오프라인 reload하면 데이터 복원 실패(실사용 온라인 체류로 무관)
